...
This dataset contains `r nrow(gm)` rows and
  `r ncol(gm)` columns.
...
There is 250 countries in the dataset.
```{r}
gm %>% 
  group_by(name) %>% 
  filter(!is.na(name)) %>% 
...
There is 249 countries with iso2 codes in the dataset.
```{r}
gm %>% 
  group_by(iso2) %>% 
  filter(!is.na(iso2)) %>% 
...
gm %>% 
  filter(!is.na(time)) %>% 
  summarize(max = max(time), min = min(time))
```
---------------------------------------------------------------------
...
ggplot(countries, aes(x = time, y = co2_PC, color = name)) +
  geom_line() +
  xlab("Year") +
  ylab("CO2 Emissions over time") +
...
ggplot(co2pc_country, aes(x = time, y = co2_PC, color = name)) +
  geom_line() +
  xlab("Year") +
...
gm %>% 
  ...
  labs(tite = "Average CO2 Emissions per Capita") +
